...
(function () {
  ...
  function wireScan() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      setTimeout(function () {
        ...
        if (grade) grade.textContent = score >= 85 ? t("hero.gradeStrong")
  ...
  var I18N = {
    en: {
      ...
      "crit.hierarchy": "Visual hierarchy", "crit.usability": "Usability", "crit.accessibility": "Accessibility",
      "crit.consistency": "Consistency", "crit.conversion": "Conversion clarity", "crit.performance": "Performance signals",
      ...
      "criteria.eyebrow": "The rubric", "criteria.title": "Six things we measure",
      "criteria.sub": "Each criterion is scored independently, so you see exactly where the points leak — and where you already win.",
      "criteria.hierarchy.body": "Does the eye land where it should? We check contrast, scale, spacing and grouping against the page's actual goal.",
      "criteria.usability.body": "Clear affordances, predictable navigation, forgiving interactions — measured against Nielsen's heuristics.",
      "criteria.accessibility.body": "Contrast ratios, focus order, target sizes and semantics, mapped to WCAG 2.2 AA where it matters most.",
      "criteria.consistency.body": "One type scale, one spacing system, one voice. We flag the drift that quietly erodes trust.",
      "criteria.conversion.body": "Is the next action obvious? We weigh CTA prominence, friction and the path from arrival to action.",
      "criteria.performance.body": "Perceived speed, layout stability and the loading experience that shapes a first impression.",
    ...
    nl: {
      ...
      "crit.hierarchy": "Visuele hiërarchie", "crit.usability": "Bruikbaarheid", "crit.accessibility": "Toegankelijkheid",
      "crit.consistency": "Consistentie", "crit.conversion": "Conversiehelderheid", "crit.performance": "Performance-signalen",
      ...
      "criteria.eyebrow": "De rubric", "criteria.title": "Zes dingen die we meten",
      "criteria.sub": "Elk criterium krijgt een eigen score, zodat je precies ziet waar punten weglekken — en waar je al goed scoort.",
      "criteria.hierarchy.body": "Landt het oog waar het moet? We checken contrast, schaal, ruimte en groepering tegen het feitelijke doel van de pagina.",
      "criteria.usability.body": "Duidelijke affordances, voorspelbare navigatie, vergevingsgezinde interacties — getoetst aan Nielsens heuristieken.",
      "criteria.accessibility.body": "Contrastverhoudingen, focusvolgorde, klikgrootte en semantiek, afgezet tegen WCAG 2.2 AA waar het er het meest toe doet.",
      "criteria.consistency.body": "Één type-schaal, één spacing-systeem, één stem. We signaleren de afwijkingen die langzaam vertrouwen ondermijnen.",
      "criteria.conversion.body": "Is de volgende stap duidelijk? We wegen CTA-prominentie, frictie en het pad van binnenkomst tot actie.",
      "criteria.performance.body": "Ervaren snelheid, layout-stabiliteit en de laadervaring die een eerste indruk vormt.",
  ...
  // ================================================================
  var CRIT_KEYS = ["hierarchy", "usability", "accessibility", "consistency", "conversion", "performance"];
  ...
  var INDUSTRY_PROFILES = {
    general: {
      ...
      weights: { hierarchy: .18, usability: .18, accessibility: .22, consistency: .14, conversion: .16, performance: .12 },
      focus: [
    ...
    ecommerce: {
      ...
      weights: { hierarchy: .14, usability: .16, accessibility: .14, consistency: .10, conversion: .32, performance: .14 },
      focus: [
    ...
    saas: {
      ...
      weights: { hierarchy: .16, usability: .20, accessibility: .14, consistency: .14, conversion: .24, performance: .12 },
      focus: [
    ...
    healthcare: {
      ...
      weights: { hierarchy: .14, usability: .18, accessibility: .32, consistency: .12, conversion: .10, performance: .14 },
      focus: [
    ...
    finance: {
      ...
      weights: { hierarchy: .14, usability: .16, accessibility: .24, consistency: .14, conversion: .18, performance: .14 },
      focus: [
    ...
    hospitality: {
      ...
      weights: { hierarchy: .14, usability: .18, accessibility: .14, consistency: .12, conversion: .24, performance: .18 },
      focus: [
    ...
    education: {
      ...
      weights: { hierarchy: .16, usability: .18, accessibility: .28, consistency: .12, conversion: .12, performance: .14 },
      focus: [
    ...
    realestate: {
      ...
      weights: { hierarchy: .16, usability: .18, accessibility: .14, consistency: .12, conversion: .20, performance: .20 },
      focus: [
    ...
    nonprofit: {
      ...
      weights: { hierarchy: .14, usability: .18, accessibility: .30, consistency: .12, conversion: .14, performance: .12 },
      focus: [
    ...
    agency: {
      ...
      weights: { hierarchy: .26, usability: .14, accessibility: .14, consistency: .22, conversion: .12, performance: .12 },
      focus: [
  ...
  function runAnalysis(html, filename, industryKey) {
    ...
    if (imgsNoAlt.length) {
      ...
      deduct("accessibility", Math.min(30, n1 * 6), n1 > 2 ? "high" : "med",
    ...
    if (unlabelled.length) {
      ...
      deduct("accessibility", Math.min(25, n2 * 8), "high",
    ...
    if (vagueLinks.length) {
      ...
      deduct("usability", Math.min(20, n3 * 5), "med",
    ...
    if (iconOnly.length) {
      ...
      deduct("usability", Math.min(15, n6 * 5), "med",
    ...
    if (inlineStyled.length > 8) {
      ...
      deduct("consistency", Math.min(20, Math.floor(n4 / 4)), "low",
    ...
    if (imgsNoSize.length) {
      ...
      deduct("performance", Math.min(20, n5 * 4), "med",
    ...
    var overall = Math.round(
      scores.hierarchy * w.hierarchy + scores.usability * w.usability + scores.accessibility * w.accessibility +
      scores.consistency * w.consistency + scores.conversion * w.conversion + scores.performance * w.performance
    );

    return { overall: overall, scores: scores, issues: issues, filename: filename || "your-page.html", industryKey: profileKey, profile: profile };
  }
  ...
  function renderResults(result) {
    ...
    subWrap.innerHTML = "";
    CRIT_KEYS.forEach(function (key) {
  ...
  function populateIndustrySelects() {
    document.querySelectorAll("#industry-select, #ff-industry").forEach(function (select) {
  ...
  function wireIndustryTabs() {
    ...
    function renderPanel() {
      ...
      var barsHtml = CRIT_KEYS.map(function (ck) {
        var w = profile.weights[ck];
        var pct = Math.round((w / maxW) * 100);
        return '<li class="ind-weight"><span class="ind-weight__name">' + t("crit." + ck) + '</span>' +
